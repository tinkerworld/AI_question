...
export const ExamsPage: React.FC = () => {
  ...
  const fetchExams = async () => {
    setLoading(true);
    try {
      let url = 'http://localhost:4000/api/v1/exams';
  ...
  const fetchPatternsAndCourses = async () => {
    try {
      const [patRes, crsRes] = await Promise.all([
        fetch('http://localhost:4000/api/v1/exam-patterns', { headers }),
        fetch('http://localhost:4000/api/v1/courses', { headers }),
      ]);
  ...
  const fetchExamDetails = async (id: string) => {
    ...
    try {
      const res = await fetch(`http://localhost:4000/api/v1/exams/${id}/draft`, { headers });
      const data = await res.json();
      if (data.success) {
  ...
  const handleCreateManualExam = async (e: React.FormEvent) => {
    ...
    try {
      const res = await fetch('http://localhost:4000/api/v1/exams/manual', {
        method: 'POST',
        headers,
        body: JSON.stringify({
  ...
  const handleSwapQuestion = async (newQId: string) => {
    ...
    try {
      const res = await fetch(
        `http://localhost:4000/api/v1/exams/${selectedExamId}/questions/${targetSwapQuestion.questionId}/swap`,
        {
          method: 'PATCH',
          headers,
  ...
  const handleRegenerateSection = async (secId: string) => {
    ...
    try {
      const res = await fetch(
        `http://localhost:4000/api/v1/exams/${selectedExamId}/sections/${secId}/regenerate`,
        {
          method: 'PATCH',
          headers,
        }
  ...
  const handleMoveQuestion = async (secId: string, qIndex: number, direction: 'UP' | 'DOWN') => {
    ...
    try {
      const res = await fetch(`http://localhost:4000/api/v1/exams/${selectedExamId}/reorder`, {
        method: 'PATCH',
        headers,
  ...
  const handleSaveSettings = async (e: React.FormEvent) => {
    ...
    try {
      const res = await fetch(`http://localhost:4000/api/v1/exams/${selectedExamId}`, {
        method: 'PATCH',
        headers,
        body: JSON.stringify({
          name: editName.trim(),
  ...
  const handlePublishExam = async () => {
    ...
    try {
      const res = await fetch(`http://localhost:4000/api/v1/exams/${selectedExamId}/publish`, {
        method: 'POST',
        headers,
      });
      const data = await res.json();
  ...
  const handleOpenQuestionPicker = async (secId: string) => {
    ...
    try {
      const res = await fetch('http://localhost:4000/api/v1/questions?status=PUBLISHED&limit=100', { headers });
      const data = await res.json();
      if (data.success) {
        const existingQIds = new Set(
          examDetails?.sections.flatMap((s) => s.questions?.map((item) => item.questionId) || []) || []
  ...
  const handleAddPickedQuestions = async () => {
    ...
    try {
      const res = await fetch(`http://localhost:4000/api/v1/exams/${selectedExamId}/questions`, {
        method: 'POST',
        headers,
        body: JSON.stringify({
